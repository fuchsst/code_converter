    ...
    def create_task(self, agent, context: str):
        ...
        return Task(
            description=(
                ...
                "- The proposed Godot project structure (in Markdown format) for this package.\n"
                f"- The target language: {config.TARGET_LANGUAGE}.\n\n"
                "Your primary goal is to create a detailed plan for converting the C++ code "
                "within this package to the proposed Godot structure. You must generate **TWO** distinct outputs:\n\n"
                "1.  **Mapping Strategy (Markdown):**\n"
                "    - Write a concise Markdown document outlining the high-level conversion strategy.\n"
                "    - Describe how key C++ classes, functions, data structures, and patterns identified in the code snippets will map to the proposed Godot nodes, scenes, scripts, signals, and specific Godot APIs.\n"
                ...
                "        - `description`: A brief description of the specific action (e.g., 'Implement Player movement logic in player.gd based on CppPlayer::updateMovement').\n"
                "        - `target_godot_file`: The relative path to the Godot script/scene to be created/modified (e.g., 'src/player/player.gd').\n"
                ...
                "    - Ensure tasks are small, focused, and provide enough detail for a code generation agent to attempt implementation.\n\n"
                "**Crucially, format your entire response so that the Markdown strategy comes first, followed by a clear separator, and then the complete JSON task list.**"
            ),
            expected_output=(
                "A single string containing:\n"
                "1. The Mapping Strategy document in Markdown format.\n"
                "2. A clear separator line (e.g., `--- JSON TASK LIST BELOW ---`).\n"
