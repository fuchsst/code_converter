...
def get_code_generator_agent(llm_instance: BaseLLM, tools: List[BaseTool]):
    ...
    return Agent(
        ...
        goal=(
            ...
            f"2. **Check Target File**: The context might indicate if the target Godot file (e.g., `{'{target_godot_file}'}`) already exists and may contain its current content. If not, or to ensure you have the latest version, you **MUST use the '{reader_tool_name}' tool** to read the current content of the target Godot file if it exists. "
        ...
        backstory=(
            ...
            f"You always use the '{reader_tool_name}' tool to fetch existing content before modification to ensure context is not lost and changes are integrated correctly. "
