...
def get_code_refinement_agent(llm_instance: BaseLLM, tools: List[BaseTool]):
    ...
    """
    # Ensure the GodotFileReaderTool is available and use its specific name.
    # This agent specifically deals with Godot files.
    godot_reader_tool_name = "Godot Project File Reader"
    if not any(t.name == godot_reader_tool_name for t in tools):
        logger.warning(f"'{godot_reader_tool_name}' not found in provided tools for CodeRefinementAgent. Agent may not function correctly.")
    ...
    return Agent(
        ...
        goal=(
            ...
            f"2. **Read Current File Content**: You **MUST use the '{godot_reader_tool_name}' tool** to read the current (erroneous) content of the `target_godot_file`. "
            ...
            f"If, after analysis, you determine that the errors cannot be fixed or are too ambiguous, you may return the original file content (obtained via the '{godot_reader_tool_name}' tool) along with a clear error message explaining the situation. "
        ...
        backstory=(
            ...
            f"You always use the '{godot_reader_tool_name}' tool to get the most up-to-date version of the file before attempting any refinement. "
